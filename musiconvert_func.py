...
class shell(object):
    ...
    def getdirs(self, path, recursive=True):
...
def getinputfilelists(indir, ext, recursive=True, verbose=True):
...
def getdirlist(indir, ext, recursive=True):
...
class audioformat(object):
    ...
    def extchecker(self):
        return shell().makeextensionchecker(self.extension)

    def istype(self, filename):
        if os.path.isfile(filename):
            return self.extchecker()(filename)
        else:
            return False
...
class flac(audioformat):
    def __init__(self, flacpath, metaflacpath, *args, **kwargs):
        ...
        self._extchecker = shell().makeextensionchecker(self.extension)

    def extchecker(self):
        return self._extchecker
