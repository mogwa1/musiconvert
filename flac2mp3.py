...
class shell:    
    ...
    def getescapedfilesperdir(self,path):
        ...
        for file in infiles:
            if(os.path.isdir(os.path.join(path,file))):
                nextdir.append(os.path.join(path,file))
            else:
                if os.path.join(path,file)[-4:].lower() == 'flac':
                    filestring += ' ' + self.parseEscapechars(
						os.path.join(path,file)) 
                    #filestring += ' ' + os.path.join(path,file)     

        if filestring != '': outfiles.append(filestring)

        for directory in nextdir:
            outfiles = outfiles + self.getescapedfilesperdir(directory)
...
def encode_thread(current_file,filecounter,opts):
    ...
    if (string.lower(current_file [-4:]) != "flac"):
        ...
        if ( os.path.exists(outfile + "." + ext) and (opts['overwrite'] == False) ):
            print "file #{} exists, skipping".format(filecounter.current)
        else:
            ...
            if (string.lower(current_file [-4:]) == "flac"):
                ...
                if (opts['mode'] != "test"):
                    ...
                    print "testing file #" + str(filecounter.current)
...
def post_processing(current_dir,dircounter,opts):
    if (opts['mp3gain'] == True):
        if glob(os.path.join(current_dir,'*.mp3')):
            dircounter.current += 1
            print 'Applying mp3gain to dir #{} of {}: {}'.format(
...
parser.add_option("-g","--mp3gain",dest="mp3gain",action="store_true",
      default=False,help="Run mp3gain album analysis on folder level")
