...
"""
A couple of functions that define crucial settings
"""

def header():
    return """MusiConvert python script, v0.2"""
...
class flac(audioformat):
    ...
    def gettags(self, infile, imgfilename):
        ...
        command = [self.opts['metaflacpath'], '--no-utf8-convert',
        ...
        if eyed3.mp3.isMp3File(outfile):
            ...
            try:
                ...
                for label in metadata:
                    metadata[label] = uni2ascii(metadata[label])
                audiofile.tag = self.settagfields(audiofile.tag, 
                                metadata, imgfilename)
                audiofile.tag.save(version=eyed3.id3.ID3_V2_3)
            try:
        ...
        for label in mp3.tag_id:
            if label in metadata:
                if mp3.tag_id[label][:4] == 'TXXX':
                    ...
                    tag.setTextFrame(mp3.tag_id[label], 
                                     metadata[label])
...
def encode_process_star(args):
    return encode_process(args[0], args[1])

def encode_process(filename, opts):
    status = ''
    ...
    outdirFinal = os.path.join(os.path.abspath(opts['outdir']),
    ...
    #####################
    # workaround for unicode filenames which are not supported by
    # subprocess.Pipe and subprocess.call
    import random
    tempbasename = os.path.basename(filename).encode('ascii', 'ignore')
    prefix = str(random.randint(0,100000))+'-'
    tempinfile = os.path.join(opts['tempdir'], 
                              prefix+'in-'+tempbasename)
    shutil.copy(filename, tempinfile)
    tempoutfile = os.path.join(opts['tempdir'], 
                               prefix+'out-'+tempbasename)
    tempoutfile_noext = os.path.splitext(tempoutfile)[0]
    tempoutfile = tempoutfile_noext  + '.' + opts['outext']
    #####################

    outfile = os.path.join(outdirFinal,os.path.basename(current_file_rel))
    ...
    if os.path.exists(outfile) and not opts['overwrite']:
        ...
        imgfile = os.path.join(opts['tempdir'], 
                               os.path.basename(tempoutfile_noext+'.jpg'))
        ...
        ######################
        # workaround for unicode filenames (part 2)
...
def albumgain_process_star(args):
    return albumgain_process(args[0], args[1])

def albumgain_process(directory, opts):
    outputformat = None
    if opts['outformat'] == 'mp3':
        outformat = mp3(opts)
    elif opts['outformat'] == 'flac':
        outformat = flac(opts)
    outformat.albumgain(directory)
    status = 'ok'
    return (directory, status)


def getinputfilelists(opts):
...
def getoutputdirlist(opts):
    # get list of directories containing converted files
    dirlist = shell().getdirs(opts['indir'], opts['recursive'])
    # filter out the directories that don't contain input files
    dirlist = [x for x in dirlist 
              if glob.glob(os.path.join(x, '*.'+opts['ext']))]
    # change input path to output path
    dirlist = [x.replace(opts['indir'], opts['outdir']) for x in dirlist]
    dirlist.sort()
    return dirlist


def copyotherlist(otherlist, opts):
    ...
    if (os.path.abspath(opts['indir']) != os.path.abspath(opts['outdir'])):
        for current_file in otherlist:
            ...
            current_file_rel = os.path.relpath(current_file, 
                                               os.path.abspath(opts['indir']))
            outdirFinal = os.path.join(os.path.abspath(opts['outdir']),
                                       os.path.dirname(current_file_rel))
            ...
            if os.path.exists(outfile) and not opts['overwrite']:
                if opts['verbose']:
                    print('File already exists ({}), skipping'.format(
                          uni2ascii(os.path.basename(current_file))))
            else:
                if opts['verbose']:
...
def convert_dir(opts):
    ...
    convertlist, otherlist = getinputfilelists(opts)
    dirlist = getoutputdirlist(opts)
    ...
    if opts['copy']:
        copyotherlist(otherlist, opts)
    
    # create temporary directory to store images, conversion files etc.
    with temporary_dir() as opts['tempdir']:
        # create pool of processes to to conversion
        pool = mp.Pool(opts['threads'])
        tasks = [(i, opts) for i in convertlist]
        results = pool.imap_unordered(encode_process_star, tasks)
        pool.close()
        for i, result in enumerate(results):
            if opts['verbose']:
                if result[1] == 'ok':
                    basetext = 'Converted {}/{}: {}'
                elif result[1] == 'ok trackgain':
                    basetext = 'Converted and track gain applied {}/{}: {}'
                elif result[1] == 'no overwrite':
                    basetext = 'File exists, skipping {}/{}: {}'
                print(basetext.format(i+1, convertnr, 
                                      uni2ascii(os.path.basename(result[0]))))
                    
        pool.join()

        # repeat similar pool tasks for directories if album gain is needed
        if opts['albumgain']:
            pool = mp.Pool(opts['threads'])
            tasks = [(i, opts) for i in dirlist]           
            results = pool.imap_unordered(albumgain_process_star, tasks)
            pool.close()
            for i, result in enumerate(results):
                if opts['verbose'] and result[1] == 'ok':
                    print('Album gain applied to {}/{}: {}'
                          .format(i+1, dirtotal, 
                                  uni2ascii(os.path.basename(result[0]))))
            pool.join()
...
def parse_arguments():
    ...
    parser.add_argument('-f', '--force', dest='overwrite', action='store_true',
                        ...
                        default=False),
    parser.add_argument('-t', '--threads', dest='threads', type=int, default=mp.cpu_count(),
                        help='How many encoding threads to run in parallel (default is # of cores).')
#    parser.add_argument('-nd', '--nodirs', dest='nodirs', action='store_true', default=False,
#                        help='Don\'t create subdirectories, put everything together '
#                        '(Default=False).')
    parser.add_argument('-nr', '--non-recursive', dest='recursive', action='store_false',
                        default=True, help='Do NOT loop trough subdirectories recursively '
                        'of the given input directory (default is recursive).')
